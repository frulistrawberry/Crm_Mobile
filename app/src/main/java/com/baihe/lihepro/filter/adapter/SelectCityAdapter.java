...
public class SelectCityAdapter extends RecyclerView.Adapter<SelectCityAdapter.Holder> {
    ...
    public SelectCityAdapter(List<FilterRegionEntity> all, List<FilterRegionEntity> select, Context context) {
        this.all = all;
        this.select = select;
        this.inflater = LayoutInflater.from(context);
    }

    public void setSelect(List<FilterRegionEntity> select) {
        this.select = select;
    ...
    public Holder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        SelectCityAdapter.Holder holder = new SelectCityAdapter.Holder(view);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull Holder holder, final int position) {
        FilterRegionEntity entity = select.get(position);
        holder.filter_city_select_name_tv.setText(getText(entity));
    ...
    public String getText(FilterRegionEntity filterRegionEntity) {
        int index = all.indexOf(filterRegionEntity);
        if (index != -1) {
            if (all.get(index).children.size() == filterRegionEntity.children.size()) {
                return filterRegionEntity.name;
            }
        }
        StringBuffer buffer = new StringBuffer();
        buffer.append(filterRegionEntity.name);
        for (FilterRegionEntity child : filterRegionEntity.children) {
