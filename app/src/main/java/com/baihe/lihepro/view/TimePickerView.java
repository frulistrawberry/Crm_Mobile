...
public class TimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    public com.baihe.lihepro.view.WheelTime wheelTime; //自定义控件
    ...
    private void initView(Context context) {
        ...
        if (mPickerOptions.customListener == null) {
            LayoutInflater.from(context).inflate(R.layout.schedule_picker_time, contentContainer);
    ...
    private void initWheelTime(LinearLayout timePickerView) {
        wheelTime = new com.baihe.lihepro.view.WheelTime(timePickerView, mPickerOptions.type, mPickerOptions.textGravity, mPickerOptions.textSizeContent);
    ...
    public void returnData() {
        if (mPickerOptions.timeSelectListener != null) {
            try {
                Date date = WheelTime.dateFormat.parse(wheelTime.getTime());
                String schedule = wheelTime.getSchedule();
                ((PickerOptions1)mPickerOptions).onScheduleSelectListener.onScheduleSelect(schedule,date, clickView);
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }

        if (((PickerOptions1)mPickerOptions).onScheduleSelectListener != null) {
            try {
                Date date = WheelTime.dateFormat.parse(wheelTime.getTime());
                String schedule = wheelTime.getSchedule();
                ((PickerOptions1)mPickerOptions).onScheduleSelectListener.onScheduleSelect(schedule,date, clickView);
