...
public class SliderRadioGroup extends RadioGroup {
     ...
     * 默认滑块颜色
     */
    private static final int SLIDER_COLOR_DEFAULT = Color.parseColor("#F096B4");
    /**
     * 默认滑块宽度所占比重
     */
    ...
    /**
     * 滑块策略间隙，当RadioGroup不是固定高时，为容下滑块而自动调整高度，该值为滑块和文字之间的间隙值，根据滑块位置策略（SliderGravity）自动分配高度
     */
    private int sliderReferSpacing;
    /**
     * 底部线区域
     */
    ...
    public SliderRadioGroup(Context context, AttributeSet attrs) {
        ...
        slideRound = typedArray.getDimension(R.styleable.SliderRadioGroup_slider_round, CommonUtils.dp2px(context, 1.0f));
        slideHeight = typedArray.getDimension(R.styleable.SliderRadioGroup_slider_height, CommonUtils.dp2px(context, 2.0f));
        sliderReferSpacing = typedArray.getDimensionPixelOffset(R.styleable.SliderRadioGroup_slider_refer_spacing, CommonUtils.dp2pxForInt(context, 2.0f));
    ...
    private void init(Context context) {
        ...
        sliderGravity = SliderGravity.CENTER;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView instanceof RadioButton) {
                ...
                childHeight = getTextHeightForInt(textPaint);
                if (childWidthRefer == ChildWidthRefer.TEXT) {
                    childWidth = getTextWidth(radioButton.getText().toString(), textPaint);
                }
            }
            if (minChildWidth == 0) {
        ...
        //如果高度不确定则自动分配克容下滑块的自身高度
        if (heightSpecMode == MeasureSpec.AT_MOST || heightSpecMode == MeasureSpec.UNSPECIFIED) {
            int heasuredWidth = 0;
            if (sliderGravity == SliderGravity.CENTER) {
                remainingHeight = slideHeight + bottomLineHeight + 2 * sliderReferSpacing;
            } else {
                remainingHeight = slideHeight + bottomLineHeight + sliderReferSpacing;
            }
            heasuredWidth = (int) (maxChildHeight + 2 * remainingHeight);
            setMeasuredDimension(getMeasuredWidth(), heasuredWidth);
        }
        if (sliderGravity == SliderGravity.CENTER) {
        ...
        if (slideBottom > 0) {
            int childLeft = childLeftMap.get(selectIndex)==null?0:childLeftMap.get(selectIndex);
            int childWidth = childWidthMap.get(selectIndex)==null?0:childWidthMap.get(selectIndex);
